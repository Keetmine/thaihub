...
const nextConfig: NextConfig = {
  output: "standalone",
  devIndicators: false,
  // Метаданные — блокирующие, а не потоковые (правка владельца
  // 2026-09-17: «в названии вкладки показывается предыдущая»). С
  // потоковой отдачей Next ставит <title> только когда страница
  // дострелилась: адрес уже новый, а во вкладке старое название
  // (локально 160–650 мс, на проде с сетью и базой заметно дольше).
  // Регулярка «любой агент» — документированный способ выключить
  // потоковые метаданные целиком; generateMetadata у нас читает те же
  // кэшированные запросы, что и страницы, так что цена — доли TTFB.
  htmlLimitedBots: /.*/,
  async headers() {
    return [
      {
  ...
  // redirects() здесь НЕТ намеренно (2026-09-16). Переехавшие адреса
  // живут в `movedTo` (src/proxy.ts): next.config сравнивает сырой путь,
  // а русские страницы приходят с префиксом `/ru`, и правило
  // `/performers` мимо `/ru/performers` промахивалось — проверено на
  // проде. В прокси путь уже без префикса, и одно правило закрывает оба
  // языка.
};
