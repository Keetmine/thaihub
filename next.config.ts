...
const nextConfig: NextConfig = {
  ...
  async redirects() {
    return [
      ...
      // «Популярное» (/dramas/top) удалено 2026-09-16: каталог и так
      // сортируется по популярности, и отдельная страница с тем же
      // смыслом стала лишней. Редирект, а не 404: страница была открыта
      // гостю, лежала в карте сайта и успела попасть в индекс — терять
      // эти ссылки нельзя, тем более пока не отыгран августовский обвал
      // трафика (docs/features/seo.md).
      {
        source: "/dramas/top",
        destination: "/dramas",
        permanent: true,
