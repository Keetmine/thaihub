...
async function expectPaywall(page: Page, url: string, feature: string) {
  ...
  await expect(page.getByRole("heading", { name: `${feature} — по подписке` })).toBeVisible();
...
test("платные разделы закрыты без подписки и открываются с ней", async ({ page }) => {
  ...
  try {
    ...
    await expectPaywall(page, "/events", "Афиша событий");
    ...
    await expectPaywall(page, "/calendar", "Календарь");
    await expectPaywall(page, "/trips", "Поездки");

    runDbScript("set-premium-test-user.ts", email);

    // С подпиской пейволл исчезает, появляются фильтры и реальные события.
    await page.goto("/events", { waitUntil: "domcontentloaded" });
    await expect(page.getByRole("link", { name: "Я иду" })).toBeVisible();
    await expect(page.getByRole("heading", { name: /по подписке/ })).toHaveCount(0);
    ...
    await expect(page.getByRole("button", { name: "+ Создать поездку" })).toBeVisible();
    await expect(page.getByRole("heading", { name: /по подписке/ })).toHaveCount(0);
  } finally {
...
test("промокод активируется один раз и открывает подписку", async ({ page }) => {
  ...
  try {
    ...
    await page.goto("/events", { waitUntil: "domcontentloaded" });
    await page.getByPlaceholder("Промокод").fill(code);
    ...
    await page.getByPlaceholder("Промокод").fill(code);
    await page.getByRole("button", { name: "Активировать" }).click();
    await expect(page.getByText("Код уже использован")).toBeVisible();
  } finally {
