...
test.describe("страницы после перестройки импортов", () => {
  test("импорты: вкладки журналов переключаются", async ({ page }) => {
    ...
    // Именно заголовок карточки импорта: с появлением журнала вкладки
    // (2026-09-06) та же подпись есть у каждой карточки прогона.
    await expect(
      page.getByRole("heading", { name: "YouTube Music: дискография" }),
    ).toBeVisible();
