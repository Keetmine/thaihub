...
**Старые версии образа удаляются сами.** Каждый пуш в main кладёт в
ghcr новый тег, и за месяц автодеплоя их набираются сотни — образ
тяжёлый, в нём chromium для парсеров. Job `cleanup-images` в конце
`deploy.yml` держит **15 последних** (`actions/delete-package-versions`):
на откат хватает с запасом, остальное не нужно никому. Работает
**после** успешного деплоя — удалять образы раньше, чем сервер их
скачал, значит остаться без отката ровно тогда, когда он понадобился;
текущий задеплоенный всегда самый свежий и под нож не попадает.

Если шаг однажды ответит 403 — `GITHUB_TOKEN` не дали прав на пакет
пользователя, лечится PAT с `delete:packages` в секретах репозитория.
Деплой к этому моменту уже прошёл, на сайте это не сказывается.
