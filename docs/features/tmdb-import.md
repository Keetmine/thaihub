...
**В имени файла есть отпечаток адреса**, и это не украшение.
`downloadRemoteImage` считает «файл с таким именем уже на диске» за
«та же картинка, качать не надо» — иначе повторный прогон тянул бы всё
заново. Пока имя целиком бралось из чужого адреса, эта проверка врала:
у WordPress-афиш файл называется по размеру («bnr_1050_486-1.jpg»), и
одним таким именем на a-ara.co.jp названы постеры ШЕСТИ разных
событий. Первое одобрение клало файл на диск, а все следующие получали
чужую картинку — и так до конца жизни файла (жалоба владельца
2026-09-25: «одобрила несколько, все с кривой картинкой, и теперь
новым событиям тоже приходит неправильная»). Теперь к имени
приписывается sha1-хвост адреса (`localImageName`): тот же адрес даёт
то же имя, разные адреса — разные файлы, и защита работает у всех
источников сразу. До этого спасал только `localBase` — своё имя,
которое вызывающий передавал вручную, если помнил о ловушке; у
одобрения черновиков оно было прописано для одного хоста из пяти.
Проверяется `tests/unit/localImageName.test.ts`. Уже созданные события
чинит `scripts/refetch-draft-posters.ts`: настоящий адрес картинки
лежит в payload черновика, поэтому постер перекачивается оттуда
(`--apply`, иначе сухой прогон; `--source a-ara` — по одному
источнику).
...
Копию сохраняем ВСЕГДА, даже когда она не легче оригинала: `srcset`
обещает браузеру файл по имени, а не по выгоде.

**`srcset` рассчитывает, что копии есть у всех картинок.** Он не
запасной путь, а выбор: взяв из него вариант, которого на диске нет,
браузер показывает дыру и к `src` НЕ возвращается. Новые файлы получают
копии при сохранении; в свежей среде догоняющий прогон
(`scripts/generate-image-variants.ts --apply`) делается один раз и
сразу, `--check` должен показать ноль. Рубильника вокруг этого нет
намеренно: `uploadSrcSet` зовут и клиентские компоненты, а клиентский
бандл видит только NEXT_PUBLIC-переменные, вшитые на сборке, — плюс в
docker-compose переменные окружения app идут через белый список.
Пробовали переменной `IMAGE_VARIANTS_READY` — не работала по обеим
причинам сразу.
