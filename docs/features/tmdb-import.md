...
**`srcset` включается отдельно — `IMAGE_VARIANTS_READY=1`.** Это не
перестраховка: `srcset` не запасной путь, а выбор. Выбрав из него
вариант, которого на диске нет, браузер показывает дыру и к `src` НЕ
возвращается. Так что до тех пор, пока копии есть не у всех картинок,
`srcset` отдавать нельзя — иначе страница остаётся без постеров. Перед
тем как включать переменную, прогоните
`scripts/generate-image-variants.ts --check`: он должен показать ноль
картинок без копий.
