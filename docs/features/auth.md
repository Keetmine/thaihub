...
**Привязка к существующему аккаунту** — виджет в настройках, маршрут
`/api/auth/telegram/link` (свой query виджет не сохраняет, из-за чего
привязка уходила в обычный вход и создавала второй аккаунт).

Виджет работает в режиме `data-onauth`: отдаёт профиль в JS-колбэк,
`TelegramLinkButton` шлёт его POST'ом, и страница остаётся на месте.
С `data-auth-url` браузер уходил на серверный колбэк и возвращался —
страница успевала перезагрузиться до того, как человек видел вопрос о
переносе. GET на том же маршруте остался запасным путём для браузеров,
где колбэк не отработал; вход на `/login` по-прежнему идёт переходом,
там перезагрузка всё равно неизбежна.

Если этот Telegram уже привязан к другому аккаунту, маршрут кладёт
подписанные данные в куку `tg_relink` и возвращает в настройки, где
показывается **попап** подтверждения (`TelegramRelinkDialog`): что
именно потеряется — избранное, отметки «иду», поездки — и кнопка
переноса. Старый аккаунт удаляется мягко (`softDeleteUser`). Раньше это
была отдельная страница, и человек терял контекст ровно в тот момент,
когда решал судьбу своего второго аккаунта. Отмена стирает куку и только потом закрывает окно: если закрыть
сразу, перезагрузка страницы в те доли секунды, пока запрос в пути,
возвращает попап — отмена выглядит неработающей. Подпись
проверяется заново при подтверждении: кука сама по себе ничего не
разрешает.
