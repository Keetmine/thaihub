...
календаря, ни «Подобрать сериал», ни «Удивите меня», ни ссылки
«Популярное» (правка владельца 2026-09-16). В своём списке они не про
то: искать там нечего — он и так перед глазами, а подбирать новое из
уже отмеченного бессмысленно. Сортировка в JS, без фильтров и страниц:
список целиком в памяти.

Ссылка на `/dramas/top` переехала из ряда вкладок в ряд кнопок разделов
каталога — оставить страницу совсем без входа нельзя, она открыта гостю
и индексируется.
...
«Популярное» ссылкой осталось в ряду вкладок «Моего списка» — на
разделах каталога вкладок нет, и одинокая ссылка в пустом ряду
читалась бы заголовком списка.
...
### «Популярное» (/dramas/top)

Страница `src/app/(public)/dramas/top/page.tsx` (аудит 2026-09,
п.6.5): сериалы по НАШЕЙ средней оценке — той же, что на странице
сериала. Среднее считает **только** `fetchSiteScores`
(`src/lib/dramaRating.ts`, правило «один человек — один голос»);
второго способа посчитать здесь нет намеренно — разъехавшиеся цифры в
топе и на карточке читались бы как баг.

- **Порог — 3 оценки на тайтл** (`MIN_VOTES`): среднее по одному
  голосу — не «наша оценка», а чьё-то мнение. Оценок на сайте пока
  немного, список выходит коротким — и это честнее длинного, надутого
  единичными десятками. Подводка называет правило прямо на странице.
- **Фолбэк**: если под порог не попадает НИЧЕГО, показывается топ по
  числу отметок «смотрю/досмотрено» (без «в планах» — туда складывают
  ещё не выбранное) с заголовком «Самое смотримое»; режим решается на
  сервере, метаданные страницы не меняются. **Подводки в фолбэке НЕТ**
  (правка владельца 2026-09-10): объяснение «оценок пока мало, поэтому
  здесь то, что чаще смотрят» — это разговор о нашей кухне, а не о
  сериалах; что список про смотримое, говорит его заголовок.
- Кандидаты — сериалы, у которых вообще есть оценки (звёзды или
  публичные отзывы, два узких `distinct`-запроса), затем среднее через
  `fetchSiteScores`; при равных средних выше тот, кого оценило больше
  людей. Потолок — 50 строк. Весь список один на всех — в
  `unstable_cache` (TTL 30 мин, тег `catalog`), как соседние гостевые
  выборки.
- Открыта гостю и **индексируема**: есть в `STATIC_PATHS` карты сайта
  (`src/lib/sitemapShards.ts`), метаданные — `pageMetadata` с
  canonical. Строка — постер-миниатюра, название, год, справа «★ 8.4
  (3 голоса)» той же подачей (цвет — `ratingColor`), в фолбэке —
  «N зрителей». Ссылка стоит вкладкой «Популярное» в ряду
  статусов каталога `/dramas` (`catalog.dramas.topLink`).
