...
справа просто текстом, без фона-карточки → события сериала →
...
`AlphabetDataList`, `DramaLocationGroups`, `SynopsisFold`) — через
`useT()`. Внутренние ссылки идут через `AppLink`, иначе со страницы
