...
постере: надзаголовок «След. серия через» с пульсирующей точкой, под
ним сам остаток («2 дня», «Завтра», «Сегодня»), ещё ниже номер серии.
Надзаголовок зависит от срока: при «Сегодня» и «Завтра» он без
«через» — «След. серия через · Сегодня» звучало бы сломанно.

Блок переделывали трижды: неброский чип в строку («сильно больше и
визуальнее»), крупная цифра в рамке («скучно как-то выводится»), и
...
нужен. Точка пульсирует, потому что это единственное на странице, что
меняется само по себе; при `prefers-reduced-motion` она просто стоит.
CSS — `.next-episode` и его дети.
...
своими звёздами), перед числом — закрашенная золотая звезда
(`.rating-star`). Цветная шкала — только у НАШЕЙ цифры: у MyDramaList
она обычного цвета строки, потому что стоит рядом просто как справка
(правка владельца 2026-09-07). Чипа «★ 7.7» над
