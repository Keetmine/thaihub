...
**Все строки — одной сеткой «подпись · значение»** (`.drama-facts`,
компонент `Fact` в `dramas/[id]/page.tsx`; правка владельца
2026-09-07: «поля с описаниями выглядят криво… надо всё выровнять»).
Раньше каждая строка была самостоятельным абзацем со своим `mb-2`,
часть строк жила во вложенном блоке со своим `gap`, а подписи стояли
внутри текста — значения не попадали в одну колонку, и строка жанров с
чипами оказывалась выше соседних, из-за чего промежуток вокруг неё
выглядел вдвое больше. Теперь колонка подписей фиксированной ширины
(7.25rem), зазор один на все строки, а чипы жанров прижаты
отрицательным полем, чтобы не растить строку. Значения — обычным цветом
текста, приглушены только подписи слева и теги (правка владельца
2026-09-07). Строка тегов — текстовым потоком (`.tag-row`), а не
flex-раскладкой: во flex «ещё N» вела себя как ещё один тег и съезжала
на отдельную строку, стоило тегам занять строку целиком. На экранах уже 576px
строка складывается: подпись сверху, значение под ней.
...
в сетке у каждого своя, иначе значения не встают в колонку.
