...
- **Один ряд с прокруткой, а не сетка** (правка владельца 2026-09-16):
  ...
  второй экран. Ряд — `PosterRow` со стрелками листания (см. ниже).
...
Ведёт на `/calendar?view=series&from=catalog`. **`from=catalog` меняет
...
самое дефицитное место — ширину на телефоне.
