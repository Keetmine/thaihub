...
**1. «Факты» — стена заметок под биографией** (`.trivia-wall`,
`.trivia-note`): каждый факт — своя карточка с акцентной подсветкой.
Путь сюда был длинным. Сначала факты переехали просто под био и
терялись в потоке текста; потом стали отдельной колонкой справа под
кнопками сердечка/глаза/плюса — и владелец отвергла и её: «когда фактов
мало, ещё более-менее, но когда много, блок слишком длинный, и чёткое
разделение на колонки как будто вышло». Данные это подтверждают: у
большинства артистов один факт, но бывает и десять, и узкий столбик
либо пустовал, либо вытягивался ниже фото.

Карточки растут вширь, а не вниз: до трёх фактов — ряд с переносом
(`flex-wrap`, последняя карточка не растягивается на всю ширину — одна
заметка на 1000px читалась бы пустой полосой); от четырёх — три
колонки (`.trivia-wall-many`, грид `auto-fit` от 11rem), где карточки
подпирают друг друга. По колонкам факты раскладывает страница
(`triviaColumns`: от длинного к короткому, каждый в самую короткую
по длине текста колонку), а
не CSS `columns`: Chrome с `break-inside: avoid` балансировал криво и
оставлял последнюю колонку пустой. Ниже 992px колонки распущены
(`display: contents`): карточки идут по две в ряд, а раз они
отсортированы по длине, соседи похожи по высоте. В рядах один длинный факт
растягивал соседей, и в коротких карточках зияла пустота. Обтекание
биографией (предложение
владельца) не взяли: при десяти фактах плавающий блок снова высокий, и
текст обтекал бы его узкой лентой. На телефоне — одна колонка.
