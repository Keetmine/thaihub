...
`/dramas` и `/novels`. Все четыре чипа обычные ссылки, включая новеллы:
снаружи разницы нет, а своя страница и свой адрес у новелл сохранились.
**Ряд стоит вплотную к списку, а не под шапкой**: он управляет именно
списком, а над витриной выглядел бы так, будто фильтрует и её.
Во время поиска не подсвечен ни один чип — ищем по всему каталогу (то
же правило, что у вкладок статуса, см. Ж5).
...
Над списком — два блока, оба видны и гостю:

- **«Новые серии»** (`src/lib/newEpisodes.ts`) — сериалы, у которых за
  последние 7 дней вышла серия, свежие сверху, до 12 штук. Запрос идёт
  по `DramaEpisode.airDate` (индекс заведён под «выходит сегодня» на
  главной), группировка по сериалу — в памяти. Границы суток —
  `startOfDay`/`endOfDay`: даты эфира лежат тайским настенным временем,
  и сравнение с моментом `now` под утро отдавало бы вчерашний день.
  Чип на постере — номера серий через `formatEpisodeNumbers`
  («13–14 серии»).
- **«Популярное»** — первые шесть строк того же топа, что и на
  `/dramas/top`. Расчёт вынесен в `src/lib/peopleTop.ts` именно ради
  этого: два способа посчитать среднее — верный способ получить
  разъехавшиеся цифры на витрине и на странице топа.

Витрина рисуется **только на разделе «Сериалы» и только без поиска**:
на «Фильмах» лента вышедших серий говорила бы не о том разделе, где
стоит человек, а в результатах поиска отодвигала бы найденное за экран.
Ниже витрины — чипы разделов, а под ними, у залогиненного, заголовок
**«Моё»** и прежний список со вкладками статусов.

### Малые разделы показываются целиком

Правило «без поиска показываем только отмеченное» (см. «Catalog scale»)
придумано для пяти тысяч сериалов. На фильмах (43) и шоу (118) оно
оставляло бы пустую страницу, поэтому такой раздел грузится целиком из
кэша (`getDramasOfKind`, порядок — год по убыванию), **без вкладок
статуса**: фильтровать сорок три строки нечего. Ссылки «Популярное» в
ряду вкладок там тоже нет — одинокая ссылка в пустом ряду читалась бы
заголовком списка.
