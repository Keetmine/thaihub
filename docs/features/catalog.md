...
вьюпорту, а буквы стоят **по центру экрана**: `position: sticky;
top: 6rem` + `height: calc(100vh - 8rem)` и `justify-content: center` —
то есть колонка занимает высоту экрана под навбаром, а центрируются
буквы ВНУТРИ неё.
Так было не всегда: сначала центрирование делали через `top: 50vh` +
`transform: translateY(-50%)`, и это давало баг — трансформа не
ограничена контейнером и поднимала рейку на пол-высоты выше её колонки,
из-за чего на `/locations` буквы ложились поверх кнопок шапки и поля
поиска. Если будете править — проверяйте, что верх рейки не выше верха
`.performers-layout`. Якоря `#letter-X` скроллят окно;
...
справа просто текстом, без фона-карточки → график выхода серий →
события сериала →
...
Блок стоит сразу за фактами — это продолжение строки «Эфир: … (по
четвергам)», только поимённо, — и ограничен по ширине 30rem: на широком
экране номер серии и дата иначе разъезжаются по краям страницы. **Серий
в базе нет — блока нет вовсе**, пустого заголовка не бывает. Справа от
заголовка — «вышло 5 из 12».
...
**Свёртка длинных расписаний.** Видно 10 строк, остальное — за кнопкой
«Показать все (N)» (при ≤2 скрываемых раскрывается сразу, как в
`CastGrid`). Окно берётся не с первой серии, а **от «сегодня»**: у
...
кнопка стоит **над** списком: список, открывающийся с четвёртой серии,
иначе выглядит обрезанным, а объяснение лежало бы под ним.

### «Сегодня выходит новая серия» на главной
...
Блок стоит **выше «Что впереди»**, сразу под приветствием: это
единственный кусок главной, который протухает за сутки, и ровно за ним
заходят утром. Без панели — голый заголовок и ряд постеров
(`PosterTile`, `col-6 col-sm-4 col-lg-3`), как у «Смотрю сейчас»:
обычно сегодня выходит один-два сериала, и в панель во всю ширину они
проваливались бы. Номер серии — чипом на постере («5 серия»); сдвоенный
показ (две строки расписания на один сериал в один день) схлопывается в
одну карточку с диапазоном («7–8 серии»). Никто сегодня не выходит —
блока нет вовсе.
