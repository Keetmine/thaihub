...
сериалы) → «Из ваших друзей смотрели» → события (фан-миты, премьеры) →
...
«Из ваших друзей смотрели» переехало под «Смотреть по порядку»: сначала
сериал и его вселенная, потом люди. События по-прежнему ниже каста и
связанных.

## «Из ваших друзей смотрели» (аудит 2026-09, п. 5.4)
...
★ 7.5»). Только для залогиненного, пустой блок не рисуется. Данные —
