...
отрицательным полем, чтобы не растить строку. На экранах уже 576px
...
**Под постером** (просьба владельца 2026-09-07) стоит чип «12 серия
выходит через 3 дня» — сколько осталось до ближайшего эфира. Чип тот же
`.date-chip` в акценте, что и «сегодня» в графике серий (CSS —
`.next-episode-chip`): одно и то же событие, одна и та же форма. От
базового чипа он отличается двумя вынужденными вещами — тянется во всю
ширину колонки постера и переносит строки: фраза в 15rem одной строкой
не помещается, а `white-space: nowrap` выпихнул бы её за край.
...
**Фраза — целиком из словаря** (`catalog.drama.schedule.nextEpisode`,
обе локали), а не «дней: 3»: это подпись на витрине. По-русски дни
согласуются через `pluralized` из `src/lib/plural.ts` («через 2 дня» /
«через 5 дней»), а нулевой и однодневный остаток называются словом —
«через 0 дней» не говорят, а «через 1 день» человек всё равно прочитает
как «завтра»:

| осталось | русский | английский |
| --- | --- | --- |
| 0 | «12 серия выходит сегодня» | Episode 12 airs today |
| 1 | «12 серия выходит завтра» | Episode 12 airs tomorrow |
| ≥2 | «12 серия выходит через 3 дня» | Episode 12 airs in 3 days |

**Разница считается календарная, а не «сколько прошло часов»**: обе даты
приводятся к UTC-полуночи (`dateKey`/`parseDateKey`), ровно как признак
«уже вышло» выше по этому же разделу. Даты эфира лежат тайским
настенным временем, и вычитание моментов давало бы 0 или 2 дня там, где
на календаре 1.

**Считает сервер, и это безопасно**: страница объявлена
`export const dynamic = "force-dynamic"`, CDN перед приложением нет и
`Cache-Control` страницам не выставляется (`next.config.ts`) — разметка
собирается на каждый запрос, застрять на сутки числу негде. Клиентский
пересчёт был бы хуже: до гидратации чип либо пустой, либо мигает.
