...
блок: надзаголовок «Следующая серия» с пульсирующей точкой, под ним сам
остаток («2 дня», «Завтра», «Сегодня»), ещё ниже тихая подпись «5 серия
· 9 сент, ср». Блок переделывали дважды: первая версия была неброским
чипом в строку («сильно больше и визуальнее»), вторая — просто крупной
цифрой в рамке («скучно как-то выводится»). Точка и тёплый градиент в
акцент — потому что это единственное на странице, что меняется само по
себе, ради него и возвращаются; при `prefers-reduced-motion` точка
просто стоит. CSS — `.next-episode` и его дети.
...
своими звёздами), перед числом — иконка звезды. Чипа «★ 7.7» над
