...
Четыре правки, после них **3334px — 3,7 экрана**.

**1. «Факты» переехали под био** (с 4562px на 717px). Это сто пикселей и
единственное человеческое на странице; по смыслу — продолжение
биографии, там им и место. Своего заголовка больше нет: в колонке фактов
он был бы третьим подряд.

**2. «Путь артиста» — ВКЛАДКА, а не ещё одна лента.** Хроника
собирается из той же фильмографии, тех же событий, альбомов и наград,
что показаны рядом: тысяча пикселей, где нет ни одного нового факта.
Теперь это второй ВЗГЛЯД на то же — «Разделы» (песни, клипы, награды по
типам) и «Путь артиста» (они же по годам), видно одно за раз. Вид живёт
в адресе (`?view=career`), а не в состоянии: срез можно переслать, и
страница остаётся серверной.

Ряд вкладок рисуется, только когда есть что переключать: у актёра без
музыки и наград хроника пуста, и переключатель был бы обманом
(`hasLowerHalf && careerItems.length > 1`).

**3. Нижняя половина — в две колонки** от 992px (`.artist-cols`).
Многоколоночная раскладка (`column-count: 2`), а не грид: блоки разной
высоты она балансирует сама, а грид оставлял бы дыру под коротким
блоком. `break-inside: avoid` у детей обязателен — без него колонка
рвёт блок посередине, и заголовок «Награды» уезжает от своего списка.

**4. Строка награды — одна строка.** Премия и категория идут через
разделитель, а не двумя абзацами, отступы поджаты. Девять наград — это
был целый экран на список, где в каждой строке десяток слов.
