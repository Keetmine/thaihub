...
Высота строки ~50px — на 900px влезает ~13 строк вместе с заголовками
букв. Вся геометрия — в co-located
`src/app/(public)/dramas/dramas.module.css`; там же уплотнены заголовки
букв (1.05rem против общих 1.4) и зазоры секций. Скоуп — класс
...
**Рейка на /dramas тоже уплотнена тем же модулем** (компонент
`AlphabetRail` не менялся): на десктопе колонка 1.1rem, кегль 0.6rem —
