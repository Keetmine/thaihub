...
полем, чтобы не растить строку. Значения — обычным цветом текста,
приглушены только подписи слева и теги.
...
раньше») — иконка, подпись и значение идут одной строкой.
...
**Под постером** (просьба владельца 2026-09-07) — крупный акцентный
блок: сверху сам остаток («2 дня», «Завтра», «Сегодня»), под ним тихая
подпись «выходит 5 серия». Первая версия была неброским чипом в строку,
и владелец попросила «сильно больше и визуальнее»: это единственное на
странице, что меняется само по себе, ради него и возвращаются. CSS —
`.next-episode` / `-value` / `-caption`, ширина по колонке постера.
...
`nextEpisodeCaption`, обе локали). По-русски дни согласуются через
...
Показываем ОДНО число — наши оценки вместе с MyDramaList
...
`data-tooltip`): «Рейтинг сериала: оценки зрителей MyBLHub — 8.9,
MyDramaList — 7.8»; при одном источнике она честно называет его один.
Точного числа наших оценок в подсказке нет намеренно (правка владельца
2026-09-07): пока их единицы, «(2 оценки)» скорее отпугивает.

**Оценки взвешены по числу голосов.** Первая версия брала ровное
среднее двух чисел, и владелец сразу поймала беду: «у MDL 7.5 на тысяче
отзывов, у нас один отзыв на 10 — и стало 8.5, будто один голос стоит
тысячи». Теперь оценка MDL входит как `MDL_VOTES` голосов, наши — как
есть. Сотня (решение владельца 2026-09-07, начинали с 20): один наш
голос итога не двигает вовсе, сотня весит наравне с MDL, дальше наши
перевешивают. Это единственная ручка, и крутится она в
`dramaRating.ts`.

Число стоит строкой в самом начале колонки фактов (над «Студия», под
своими звёздами) и бейджем у названия в строках каталога. Чипа «★ 7.7»
над постером больше нет — то же число рядом, и дубль владельца
раздражал (правки 2026-09-07).

**Цвет оценки** — общий `src/lib/ratingColor.ts`: 7+ жёлтая
