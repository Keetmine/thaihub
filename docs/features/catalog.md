...
- Выбор произвольной даты — `EpisodeDayPicker`: кнопка-календарь, под
  которой лежит нативный `input[type=date]`. Свой `DatePickerInput` тут
  не годится — его выпадашка с месяцем и годом перевесила бы сам блок.
  Прятать поле через `display:none` нельзя: скрытому полю браузер
  календарь не откроет, поэтому оно прозрачное и растянуто на кнопку.
...
Рядом с заголовком — кнопка **«Расписание»** в календарь серий
(`/calendar?view=series`). Именно кнопкой с подписью: иконку у поля
поиска никто не находил (жалоба владельца). На остальных разделах
иконка в ряду вкладок остаётся — там кнопки нет.
