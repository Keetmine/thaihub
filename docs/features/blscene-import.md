...
**Названия у blscene свои, поэтому опознание в три ступени** (вторая
жалоба владельца 2026-09-06, «Gelboys Season 2» и «там ещё много
таких»):

1. по сохранённому `blsceneUrl` — точное совпадение;
2. по НОРМАЛИЗОВАННОМУ названию (`normalizeDramaTitle`: регистр, знаки
   и слово «сезон» не в счёт) — у blscene «GELBOYS 2», у нас «Gelboys
   Season 2»;
3. по ссылке на MyDramaList со скачанной страницы
   (`findDramaByMdlUrl`, сравнение по числовому id) — она видна только
   на самой странице, поэтому ступень третья: первые две обходятся
   индексом, без единого лишнего запроса. Так узнаётся «Peach & Me»
   против нашего «Peach and Me».

Без этого «GELBOYS 2» завёлся бы ВТОРОЙ записью рядом с нашим сериалом
— то есть дублем в каталоге. Замер на живом индексе: из 116 страниц по
адресу и названию не узнавалась одна, и та нашлась по ссылке MDL;
записей, которые завелись бы заново, — ноль. Прогон локаций теперь
идёт по всему индексу и связывает найденное, но сериалов НЕ создаёт —
это по-прежнему работа синка.
