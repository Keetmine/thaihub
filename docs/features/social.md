...
- **Правая колонка**: вкладки (`ProfileTabs.tsx` — клиентское
  ...
  - **Сериалы** — под-табы по статусам `WATCH_STATUS_ORDER`
    (Смотрю/Просмотрено/…, с числом), строки компактные — та же
    плотность, что каталог `/dramas` (`.profile-drama-row`: мелкая
    миниатюра, название (`dramaTitleForLocale`) + прогресс
    (`episodeProgress`) одним потоком, дата отметки справа);
