...
Аватарка круглая, а снимки с телефона вертикальные — поэтому своё фото
человек кадрирует сам, до отправки. В настройках (`/account/settings`)
...
- В окне — квадратная резиновая рамка с затемнением вокруг круга
  (`.image-crop-mask` вырезает середину маской) и тонкой обводкой
  круга: видно и будущую аватарку, и то, что в неё не влезло.
...
- По «Поставить фото» кроп рисуется на `canvas` и уходит на
  `/api/upload` уже квадратом: сторона — не больше 1024, вверх исходник
  не растягивается; формат — WebP (`toBlob`), в который ручка всё равно
