...
- **Бронирования** (`TripBooking`, enum `TripBookingKind`: `HOTEL` |
  ...
  **Две стороны без ничего между ними схлопываются в одну строку**
  ...
  маршрут или адрес и вторая дата словами: у отеля «12–15 мар · 3 ночи»
  (`formatShortDateRange` в `lib/dates.ts`), у перелёта — «прилёт
  11 мар» (`arrivesOn`), только если сел не в день вылета. Заметка у
  ...
  **Цепочка перелётов — одной строкой** (выбор владельца из двух
  ...
  `fromPlace`/`toPlace` без повторов подряд («Минск → Москва → Хайкоу →
  Бангкок»), «2 пересадки (Москва 5 ч 20 мин, Хайкоу 3 ч 40 мин)»
  (`layovers(n)` в словарях, длительность — `formatDuration` в
  `lib/dates.ts`) и «прилёт 5 апр», если последний сегмент сел не в день
  первого вылета. **Общего времени в пути нет намеренно**: времена
  ...
  прилёта или назавтра) и показывается одним местом, без длительности
  («2 пересадки (Москва, Хайкоу)»). Кнопки билета, правки и удаления у
