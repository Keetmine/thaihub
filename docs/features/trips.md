...
- **вещь и покупку** — прямо в их вкладке: `TripTodoQuickAdd` (поле и
  «+», Enter добавляет и оставляет фокус на месте — чемодан набивают
  десятком строк подряд, и модалка на каждую была мучением), а рядом
  «С датой…» — та же модалка со всеми полями. Подписи обеих кнопок
  постоянные.
