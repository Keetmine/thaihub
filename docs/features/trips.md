...
Вкладка **«Не забыть»** (между «Афишей» и «Что посетить»; видна тем, кто
вносит, и тем, кому видно хоть один пункт): обычный туду — чекбокс
выполнения, правка/удаление по правам выше. Называлась «Списки» ровно
один день — владелец: «непонятно, что это и зачем»; «Не забыть»
объясняет себя и накрывает все три списка сразу (не забыть сделать,
взять, купить). Под заголовком стоит строка-пояснение, а сегменты
показывают «сделано/всего» одинаково у всех трёх — правило одно,
объяснять нечего.

**Списков внутри неё три** — дела, чемодан («что взять с собой») и
покупки («что купить в Таиланде»), АА10/АА11. Различает их
`TripTodo.kind` (`TODO` | `PACKING` | `SHOPPING`, по умолчанию `TODO` —
всё заведённое раньше осталось делами). Отдельными моделями чемодан и
покупки не стали намеренно: галочка «готово», дата, видимость, автор и
право правки у всех трёх одинаковые, и три копии этой механики
разъехались бы с первой же правкой.

Открытый список живёт в АДРЕСЕ (`?view=todos&list=packing|shopping`), а
не в состоянии компонента: ссылкой на покупки удобно кинуть попутчице.
Сегменты нарисованы кнопками внутри вкладки, а не тремя вкладками
верхнего ряда: там уже четыре, и «Чемодан» рядом с «Афишей» читался бы
как равный ей раздел.

**Добавляют пункт ВНУТРИ списка, а не кнопкой над вкладками** (правка
владельца 2026-09-06; до этого кнопка «+ Дело» жила в общем ряду
действий — там она стояла в отрыве от того, куда добавляет, и меняла
подпись вслед за сегментом, «дёргаясь» под курсором). Внутри две вещи:
- `TripTodoQuickAdd` — поле и «+», Enter добавляет и оставляет фокус на
  месте: чемодан набивают десятком строк подряд, и модалка на каждую
  была мучением. Видимость берётся по списку (чемодан — приватный),
  остальное правится потом в самом пункте;
- «С датой…» (`AddTripTodoButton`) — прежняя модалка со всеми полями.
  Подпись у неё ПОСТОЯННАЯ, какой бы сегмент ни был открыт.
...
пункты одного вида. **Добавляют дело по кнопке «+ Дело»**
...
(просьба владельца). Сама кнопка живёт **в общем ряду действий над
вкладками**, а внутри вкладки её нет (просьба владельца): дело заводят и
с плана, и из «Что посетить», а две одинаковые кнопки на одном экране
дублировали друг друга. Поэтому `TripTodos` — только список: `canAdd`
там остался ради подсказки в пустом состоянии. Датированные дела попадают
