...
**Второй рубеж — фоновые задачи вообще не заводятся вне production**
(`src/instrumentation.ts`, правка 2026-09-10). Одной глушилки в
`sendTelegramMessage` оказалось мало: планировщик стартует один раз при
старте процесса и держит захваченные модули, поэтому dev-сервер,
поднятый ДО правки, продолжал слать старым кодом — hot reload подменяет
модули у обработчиков запросов, а не у уже тикающих таймеров. Теперь на
машине таймеры не создаются вовсе; заодно локальный компьютер перестал
параллельно с сервером ходить на ThaiTicketMajor и musicfestival.

Обе правки вступают в силу только **после перезапуска dev-сервера** —
по той же причине, по которой вторая понадобилась. Проверить
планировщик у себя — `RUN_BACKGROUND_JOBS=1`; разовый запуск задачи из
админки работает всегда, он идёт обычным запросом.
...
## Обратный отсчёт до поездки (`trip-countdown`)

Задача планировщика, а не получасовой прогон: «доброе утро, осталось
N дней» должно приходить утром (`defaultHour: 10`, час правится на
`/admin/schedule`), а не в полночь, когда сменилась дата. Функция —
`sendTripCountdowns` в `src/lib/telegramNotifications.ts`, правило и
подписи — в [trips.md](trips.md), «Обратный отсчёт до поездки».
Получают владелец и принявшие приглашение участники, каждому — до его
собственных дат (`TripStay`), если они указаны. Через `notifyUser`
(колокольчик + Telegram по `tgNotifyTrips`). Дедуп —
`TripCountdownNotification` по календарному дню: одно сообщение
человеку на поездку в день, ручной запуск задачи ничего не дублирует;
отметка ставится до отправки, гонку судит первичный ключ. Ротируется
чисткой вместе с остальными отметками (180 дней).
