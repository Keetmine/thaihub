...
- аккаунту больше трёх дней;
- есть о чём писать: человек что-то смотрит, куда-то идёт или с кем-то
  дружит. Предлагать уведомления тому, кто ничего не отметил, — значит
  рекламировать пустоту.

Само окно всплывает не сразу, а через 12 секунд на странице: попап в
лицо в момент открытия — ровно то, за что их не любят. Любое закрытие
(крестик, «Потом», клик мимо) считается ответом и записывается.
