...
**Переводы тоже сливаются** (жалоба владельца 2026-09-26: «слила два
дубля — русский перевод не подтянулся»). До этого json `translations`
в список не входил, и русская биография и факты проигравшего пропадали
вместе с его записью. Теперь `mergeTranslations` сливает его по языку и
по полю: выживший главнее, недостающее берётся у проигравшего. У
сериала своего json нет — русское там в колонках `titleRu` и
`synopsisRu`, и они тоже добавлены в список (раньше пропадали так же).
У артистов в список вошли и поля из профилей kprofiles: группа крови,
MBTI, автограф. Отметки «текст наш» (`TextRewrite`) проигравшего
переезжают к выжившему, если у того по этому полю своей нет.

Уже слитые раньше дубли так не починить: проигравших записей нет, и их
переводы остались только в бэкапах базы.
