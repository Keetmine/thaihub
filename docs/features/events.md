...
Третья вкладка на /calendar («Все события / Мои события / Дни
рождения», `?view=birthdays`): та же месячная сетка, но в ячейках —
именинники (все исполнители с `birthDate`: актёры, маскоты, группы),
чипами с аватаркой и именем, ссылкой на страницу артиста; тултип
показывает исполняющийся возраст, до 3 на день + «+N ещё». Месяцы
сетки выбираются raw-запросом по EXTRACT(MONTH) — тянуть все 550+
дат ради одного месяца незачем.
...
неё выводятся и все её участники (мини-карточки с подписью-названием
