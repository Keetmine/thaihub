...
Введённое время приводит к «ЧЧ:ММ» общий `normalizeTimeValue`
(`src/lib/dates.ts`), и он ТЕРПИМ к тому, как поле выглядит у человека:
`type="time"` рисует браузер по настройкам системы, и разделителем там
бывает точка. «12.30», «9:30», «12:30:00» и даже «1230» принимаются
одинаково; не время — это ошибка, пустое — «время не указали».
Раньше проверка была `^\d{2}:\d{2}$`, и правка минут в поле с точкой
кончалась сообщением про неправильную дату (находка владельца
2026-09-09), а `combineDateTime` в этом случае складывал Invalid Date —
в делах поездки из-за этого молча пропадала введённая дата. У каждого дня может
