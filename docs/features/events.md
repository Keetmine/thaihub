...
## Схема зала и бенефиты (Ж9)

`EventPhoto` (`eventId` cascade, `kind: SEATING | BENEFITS`, `url`,
`caption?`, `sort`) — типизированные фото, НЕ общая галерея (решение
владельца: схемой зала с ценами по секциям и картинками «что дают за
какой билет» пользуются при покупке, их надо находить по заголовку, а
не листать в карусели). Постер остаётся отдельным полем — он идёт в
OG-картинку и карточки.

Публичная страница события: «Схема зала и цены» и «Бенефиты билетов»
В РЯД (две колонки, на телефоне друг под другом), НАД блоком «Мои
билеты», без подложки-surface (правки владельца после первой версии
внизу страницы; заодно подложка снята и с «Описания»). Картинка —
среднее превью (`.event-photo-preview`, до 26rem), клик открывает
оригинал в новой вкладке; подпись — под картинкой. Блок без фото не
рендерится.

Админка: секция «Схема зала и бенефиты» в `EventForm` — два списка
`EventPhotosField` (мультизагрузка через `/api/upload`, подпись,
порядок стрелками, удаление). В форму список уходит одним hidden-полем
с JSON (`seatingPhotos`/`benefitPhotos`); экшены пересобирают строки
целиком (`deleteMany` + `create`), как остальные связи события.
