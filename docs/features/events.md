...
- **`EventCard`** (`src/components/EventCard.tsx`) — the main browsing
  ...
  Внутри дня дату несёт только ПЕРВАЯ строка (`hideDate`): при месячной
  группировке число иначе повторялось у каждого события одного дня.
  Место под дату сохраняется всегда — без него постеры соседних строк
  разъехались бы по горизонтали, и остальные строки дня читаются как
  висячий отступ под первой.
