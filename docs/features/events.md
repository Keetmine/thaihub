...
**Поля времени во всех формах заполнены нулями** (правка владельца
2026-09-09). Пустое `type="time"` браузер рисует призрачным «12:30» —
значения в форме нет, а на экране оно есть; человек правит эту фантомную
подпись, отправляет половину («12» без минут) и получает ошибку про
неправильную дату. Поэтому:

- поля стартуют с `00:00`, и показанное совпадает с тем, что уйдёт на
  сервер;
- `optionalFormTime` (`src/lib/dates.ts`) читает `00:00` из
  НЕОБЯЗАТЕЛЬНОГО поля как «время не назначено» — иначе каждая встреча и
  каждое событие получили бы настоящее начало в полночь и показывали бы
  её в афише, карточках и календаре;
- `normalizeTimeValue` приводит к «ЧЧ:ММ» и половину введённого: «12» и
  «12:» → 12:00, «:30» → 00:30, «1230» → 12:30, «12:30:00» → 12:30.
  Раньше проверка была `^\d{2}:\d{2}$`, и всё это падало в ошибку про
  дату, а `combineDateTime` складывал Invalid Date — в делах поездки
  из-за этого молча пропадала уже введённая дата.

Цена решения: событие, которое ДЕЙСТВИТЕЛЬНО начинается в полночь,
этими формами не задать — его начало ставят на 00:01. Ночных начал в
афише не бывает, а пустых полей времени — постоянно. У каждого дня может
