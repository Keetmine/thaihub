...
**Поповер не закрывается от кликов по календарю** (жалоба владельца
2026-09-22: «тыкаю на месяц или след/пред месяц — закрывается
календарь»). Сам календарь `DatePickerInput` рисуется в ПОРТАЛЕ, в
дереве `body`, — иначе он не смог бы лечь поверх модалок. Для
обработчика «клик мимо» у поповера это выглядело кликом снаружи, и
стрелка «следующий месяц» закрывала весь фильтр. Теперь обработчик
пропускает всё, что лежит внутри `.date-picker-dropdown`; список
месяцев и годов (`PickerSelect`) рисуется внутри календаря, поэтому
одной проверки хватает на все случаи.
...
`isOnline` — только у встреч сообществ (venue у них при этом пустой,
на карточках — бейдж «Онлайн»); у каталожных событий всегда `false`,
админка признак не показывает — см.
[communities.md](communities.md), раздел «Онлайн-встречи».
...
## «Идут с сайта» на странице события

На `/event/[id]` (и каталожного события, и встречи сообщества) — ряд
круглых аватарок всех людей сайта, отметивших «иду» на любую из дат:
витрина «тут есть люди». Компонент —
`src/app/(public)/event/[id]/SiteGoersBlock.tsx`, манера та же, что у
участников сообщества (`MembersBlock`) и друзей в профиле: кружки
`.profile-friend` по 2rem, подсказка-имя (`data-tooltip`), ссылка на
профиль. Первые 10 аватарок, дальше кружок «+N» с подсказкой «и ещё N»;
рядом с заголовком — общее число. Пустой ряд не рисуется вовсе.

Блок виден и **гостю** — в отличие от соседних «Друзья идут» и «Из
вашего сообщества идут», которые про своих и без логина не существуют.
На встрече сообщества отдельного гейта нет: посторонних туда не пускает
сама страница (`canSeeMeetup`).

Правила ряда (собирается в `page.tsx`, см. комментарий у `siteGoers`):

- закрывшие профиль (`User.hideProfileActivity`) не показываются — тот
  же мастер-выключатель, что в профиле (`showActivity` в
  `users/[id]/page.tsx`) и в «Из вашего сообщества идут»; сам зритель
  видит себя ВСЕГДА, даже закрывшись, и стоит в ряду первым;
- друзья и соседи по сообществу, уже показанные плашками выше, в ряд не
  дублируются (одно лицо на странице дважды — ошибка, не забота);
- удалённые аккаунты (`deletedAt`) не показываются нигде.

Отдельного запроса у блока нет: страница делает **одну** выборку
`EventAttendance` по событию (с данными пользователей), и из неё же
достаёт свои отметки зрителя и «Друзья идут» — раньше это были два
отдельных запроса, так что страница стала легче, а не тяжелее.

**Время глагола — по дате** (правка владельца 2026-09-17): отметку «иду»
ставят и задним числом, и о вчерашнем концерте страница говорила
«Друг идёт». Когда все даты события прошли, заголовок блока — «Друг
побывал(а)» / «Друзья побывали»; та же развилка у подписи на карточках
списков (`oneFriendWent` / `manyFriendsWent` в `EventCard` и
`EventAgendaRow`) и у уведомления друзьям — для прошедшей даты уходит
отдельный вид `FRIEND_ATTENDED` («X побывал(а) на …», см.
[notifications.md](notifications.md)). «(а)» — принятый на сайте
нейтральный вид прошедшего времени: пол человека мы не знаем.
...
**Импорт с ThaiTicketMajor заполняет их сам** (правка владельца
2026-09-22: «брать три картинки и вставлять их в фото для
покупателей»). TTM держит этот блок в `/cmsimg/imgeditor/` — там ровно
три картинки: план зала с ценами, что входит в билет и условия
трансляции. Парсер (`parseTtmPhotos` в `lib/thaiticketmajor.ts`) берёт
только этот путь: соседние `/img_poster/` — афиши ЧУЖИХ концертов, а
`/img_seat/` — нечитаемые превью зон. Порядок — по номеру в имени файла
(«r 02_…_SeatPlan.jpg»), в разметке он бывает любым; имена с пробелами
кодируются, иначе скачивание падает. Экран проверки импорта показывает
найденное миниатюрами с галочками — ненужную картинку снимают там же, —
а `createEventFromTtmImport` качает выбранные к себе
(`downloadRemoteImage`, своя основа имени у каждой: файлы на TTM зовутся
одинаково по всему сайту) и кладёт в `EventPhoto`. Не скачалась —
пропускаем: галерея не повод ронять импорт. Одобрение черновиков обхода
афиши идёт тем же путём, картинки лежат в payload черновика.
...
## Таймзона события и зрителя

Время в `EventOccurrence.startsAt/endsAt` лежит «настенными часами» в
UTC-полях **в зоне события** — `Event.timezone` (IANA, с 2026-09-17).
Каталожные события — `Asia/Bangkok` (умолчание колонки и
`EVENT_DEFAULT_TIMEZONE`), встречи сообществ наследуют
`Community.timezone` при создании и правке (см.
[communities.md](communities.md)). До этого все события считались
тайскими, и встреча в Беларуси показывалась как «22:10 (МСК 18:10)» —
время, введённое по Минску, сайт читал как бангкокское (жалоба
владельца).

«В скобках» — конвертация в таймзону зрителя из настроек
(`User.timezone`, дефолт `Europe/Moscow`; селект в Настройки → Профиль,
список зон в `src/lib/timezones.ts`). У тайского события подпись
прежняя: «22:10 (МСК 18:10)». У события в другой зоне время идёт со
своей подписью — «18:10 Минск» — и скобки со зрительской зоной
добавляются только когда смещения различаются (`sameOffset`): «18:10
(МСК 18:10)» было бы шумом. В списках — тултип `MskTimeInfo` /
`TzTimeText` (клиентские, зона зрителя из контекста `TimezoneProvider`,
зона события пропом `timezone`; текст — `card.thaiTime` или
`card.zoneTime`); на странице события — инлайн через
`formatTimeWithZone`/`formatTimeRangeWithZone` (`src/lib/dates.ts`,
последний аргумент — зона события). Конвертация:
`wallClockToInstant(d, eventTz)` (Бангкок — константа −7ч, остальные —
смещение через Intl с поправкой на границу DST) → Intl с `timeZone`
зрителя. Выгрузка в календарь (`lib/ics.ts`) снимает смещение той же
зоны события. Списки, где встреч не бывает (каталог, календарь),
поле не тянут — там всегда Бангкок. Админка остаётся на жёстком МСК.
...
участники пейринга ставятся рядом и **первыми** (АА4,
`keepPairingsTogether({ pairsFirst: true })` — там же в
`castLineup.ts`; правка владельца 2026-09-15, то же правило, что на
странице сериала): состав открывается парами, ради которых на концерт
и идут, а не самым «событийным» артистом второго плана. В «Лайнапе по
дням» — так же; первые ~14 видны сразу,
остальные за «Показать всех (N)». Пейринги остаются чипами под
сеткой, «Лайнап по дням» — той же сеткой без свёртки. Порядок секций
страницы: даты/билеты → «Мои билеты» → «Друзья идут» → «Из вашего
сообщества идут» → «Идут с сайта» → состав → заметки → описание →
источники → отзывы; при ≥3 якорных секциях
...
- **Карточка события `/event/[id]`** — что, когда, где, кто выступает,
  ...
  препродажи, сериал, отзывы и комментарии. Плюс ряд «Идут с сайта»
  (см. одноимённый раздел выше) — он нарочно открыт и гостю: живые
  люди на событии и есть довод создать аккаунт. И `Event`-разметка
...
Отметку «иду» стережёт и СЕРВЕР: `toggleGoing` проверяет подписку сам
(правка 2026-09-09) — раньше проверка была только в разметке, а экшен
вызывается и напрямую, мимо страницы. Два исключения намеренные:
**снятие** отметки свободно (у истёкшей подписки человек иначе остался
бы с планами в календаре и без способа их убрать — то же правило, что у
своих списков) и **встречи сообществ** подписки не требуют вовсе, ровно
...
1. **название** — белым, самый крупный элемент строки. У **встречи
   сообщества** рядом с названием стоит тихий чип с названием
   сообщества (`.event-row-community`, ссылка на само сообщество,
   `EventCard`): на вкладке афиши «Сообщества» без него было не
   понять, ЧЬЯ это встреча (жалоба владельца 2026-09-08), а карточка
   при этом обязана была остаться похожей на обычную (просьба
   владельца — «одинакового вида»), поэтому чип, а не отдельная
   строка. Данные — необязательное поле `community` у
   `EventWithPerformers`: его тянет `src/lib/eventList.ts` (афиша) и
   прокидывает `flattenOccurrence`; у каталожных событий там `null`,
   у списков без встреч (локация, сериал, артист) поля нет вовсе.
   Тянется во ВСЕХ фильтрах афиши, а не только в `communities`: во
   вкладке «Я иду» встречи и каталожные события идут вперемешку.
   Ловушка: обрезка длинного названия (`overflow: hidden`) живёт на
   внутреннем `.event-row-community-name`, а не на самом чипе — на
   чипе она срезала бы его же подсказку (`::after`);
...
интерфейсе (карточки, agenda, страница события).

**Поле времени — своё, а не `type="time"`** (`components/TimeInput.tsx`,
правка владельца 2026-09-09). Нативное поле рисует БРАУЗЕР, и пустое он
показывал как «12:30»: значения в форме нет, а на экране оно есть, и
стереть его нельзя — при попытке подпись возвращалась. Дальше человек
правил эту фантомную надпись, отправлял половину («12» без минут) и
получал ошибку про неправильную дату. По той же причине у нас своё поле
даты (`DatePickerInput`).

Здесь пустое поле пустое, подсказка формата — обычный placeholder
«чч:мм», а ввод идёт ПО МАСКЕ: принимаются только цифры, двоеточие
ставится само, часы держатся в 00–23, минуты в 00–59 (набрать «99999»
нельзя — жалоба владельца 2026-09-09). Стирая двоеточие, человек стирает
и цифру перед ним, иначе маска возвращала бы его на место и поле
«залипало». Недобранное дополняется на потере фокуса общим
`normalizeTimeValue` (`src/lib/dates.ts`) — тем же, что и на сервере:
«12» и «12:» → 12:00, «:30» → 00:30, «1230» → 12:30, «12:30:00» →
12:30. Раньше проверка была `^\d{2}:\d{2}$`, и всё это падало в ошибку
про дату, а `combineDateTime` складывал Invalid Date — в делах поездки
из-за этого молча пропадала уже введённая дата.

Смысл значений не изменился: пустое — «время не назначено»
(`hasTime = false`), 00:00 — настоящая полночь. У каждого дня может
...
**Лайнап дня — полноценный состав, а не украшение.** Артист, стоящий
только в лайнапе дня, виден на своей странице (в афише артиста
объединяются три источника: общий состав события, лайнапы дней и
события ГРУПП, в которых артист состоит) и попадает в рассылку «у
избранного артиста новое событие». До 2026-09-15 не работало ни то, ни
другое — жалоба владельца: «добавляем группы в лайнап по дням — не
приходит уведомление, и у участников группы этот эвент не отображается».
Уведомления шлют все три пути: админ-форма (`createEvent`/`updateEvent`
берут и `performerIds`, и артистов лайнапов), импорт по ссылке и
краулер musicfestival — последний зовёт рассылку по тем, кого впервые
привязал к событию. Группы раскрываются до участников внутри самой
рассылки: избравший участника LYKN ждёт весточки о концерте LYKN.
...
заявленных не стояли ни в одном дне — и страница не показывала их
нигде: ни в расписании, ни в составе (жалоба владельца 2026-09-19:
«группа заявлена на этом событии, а я её не могу найти»; отдельно то
же случилось с SERIOUS BACON, у которого слот унесло слияние дублей,
см. [duplicates.md](duplicates.md)). Поэтому под расписанием идёт блок
**«Ещё в составе»** — те из общего состава, кого нет ни в одном
лайнапе, теми же капсулами и с теми же глазиками «видела здесь».
Расписание покрывает всех — блока нет.
...
У карточки артиста в расписании есть **глазик «видела здесь»**, а у
дня — кнопка «Видела всех / Никого»: отметка «видела вживую» привязана
к событию, и ставят её прямо здесь, а не на странице каждого артиста
(см. [gamification.md](gamification.md)). Видны они только тому, у кого
на этой дате стоит «иду» и она уже прошла.
