...
- `tags` — чипами `.tag-chip` под фактами, перед составом. Чипы глухие:
  ветки поиска по тегам СОБЫТИЙ (в отличие от сериалов) нет.
...
Публично день раскладывается по сценам (`groupLineupByStage` в
`src/lib/castLineup.ts`): сцена — чип-заголовок, время — подпись под
именем на карточке. Сцены идут по первому выступлению, внутри сцены —
по времени, у кого времени нет — в конец своей сцены, а участники без
сцены — в конец дня. Слоты раньше шести утра считаются концом
