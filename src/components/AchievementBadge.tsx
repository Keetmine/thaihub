...
export type AchievementBadgeProps = {
  ...
  iconOnly?: boolean;
  /** Сколько людей получили эту медаль — строка «есть у N человек» в
...
/** «Есть у N человек» — без словаря: компонент рендерят и сервер, и
 *  клиент, а хуки словаря с сервера не зовутся (см. locale выше).
 *  «Человек», а не «фанатов» (правка владельца 2026-09-17: «более
 *  обще»). */
function holdersLabel(n: number, locale: Locale): string {
  if (locale === "en") return `${n} ${n === 1 ? "person has it" : "people have it"}`;
  // После «у» — родительный: «у 1 человека», «у 2 человек», «у 21 человека».
  const one = n % 10 === 1 && n % 100 !== 11;
  return `есть у ${n} ${one ? "человека" : "человек"}`;
}
