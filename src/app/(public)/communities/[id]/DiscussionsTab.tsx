  ...
  const [community, posts] = await Promise.all([
    ...
    prisma.communityPost.findMany({
      ...
      select: {
        id: true,
        title: true,
        // Снаружи текст темы не тянем из базы ВОВСЕ (аудит 2026-09,
        // п.1.4): обещание витрины — «заголовки публичных тем без
        // содержимого», а у темы без заголовка вывеской служит начало
        // текста — и первые 90 символов разговора уезжали постороннему.
        // Чего нет в выборке, то не утечёт и через разметку.
        ...(locked ? {} : { text: true }),
        pinned: true,
    ...
    <div className="d-flex flex-column gap-2">
      ...
      {posts.length === 0 ? (
        ...
        posts.map((post) => (
          <PostCard
            key={post.id}
            // Снаружи текста в строке нет (null): у темы без заголовка
            // PostCard покажет честное «Тема без названия», а не отрывок.
            post={{
              ...post,
              text: "text" in post ? post.text : null,
