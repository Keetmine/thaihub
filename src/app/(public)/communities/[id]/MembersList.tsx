 ...
 *  Одна на все три списка — участники, приглашённые и убранные
...
/**
 * Полный список людей сообщества. Рендерится только тем, кто внутри:
 * список людей — это персональные данные, наружу он не уходит (см.
 * `communityAccess`).
 *
 * Жил отдельной вкладкой «Участники», пока владелец её не убрал
 * (правка 2026-09-09): у людей в левой колонке и так есть сетка
 * аватаров, а вкладка ради второго их же списка занимала место в ряду.
 * Теперь этот список открывается из той сетки по «Смотреть всех» —
 * `MembersBlock` берёт его готовым узлом и показывает в окне.
 *
 * Три списка в одном месте, и это намеренно: участники, позванные и
 * убранные — ответы на один вопрос «кто здесь», просто в разных
 * состояниях. Разносить их значило бы прятать от владельца половину
 * картины.
 *
