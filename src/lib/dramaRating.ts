 ...
 *
 * Оценок у сериала три вида, и путать их нельзя:
 *
 ...
 *
 * Наша и чужая живут ОТДЕЛЬНО (решение владельца 2026-09-07). Недолгое
 * время они сводились в одно число со взвешиванием по голосам, но так
 * непонятно, чей это рейтинг, а вес чужих тысяч отзывов приходилось
 * выдумывать. Теперь наш считается только по нашим голосам, и рядом с
 * ним показывается, сколько их.
 *
...
export type DramaScore = {
  ...
  siteCount: number;
  /** Оценка MyDramaList — как есть, отдельной строкой. */
  mdl: number | null;
};
...
/** То же для одного сериала — вместе с оценкой MyDramaList. */
