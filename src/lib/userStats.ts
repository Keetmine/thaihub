...
export async function computeUserStats(
  ...
  const [
    ...
    personalEventSeen,
      ...
      // его увидеть, — и только по СОБСТВЕННОЙ отметке «видела»
      // (`TripPersonalEventSeen`, 2026-09-19: в клубе были все, но
      // одного актёра видела я, другого — подруга; кто добавил артиста,
      // у того он отмечен сразу, у остальных — нет). Автору отметки
      // ставятся при сохранении записи, бэкфилл покрыл старые записи по
