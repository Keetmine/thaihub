...
export const catalog: Dict["catalog"] = {
    ...
    /** «Популярное» /dramas/top (аудит 2026-09, п.6.5; переименовано по
     *  правке владельца 2026-09-10 — «народный топ» звучал как конкурс,
     *  а страница отвечает на вопрос «что тут смотрят») — см. комментарий
     *  в en/catalog.ts. */
    dramasTop: {
        metaTitle: "Популярные сериалы",
        metaDescription:
            "Сериалы с самой высокой средней оценкой зрителей MyBLHub — по настоящим оценкам живых людей.",
        title: "Популярное",
        intro: (min: number) =>
            `По средней оценке зрителей MyBLHub. В топ попадают сериалы, которые оценили хотя бы ${min} человека — поэтому список короткий и честный.`,
        votes: (n: number) => `${n} ${plural(n, ["голос", "голоса", "голосов"])}`,
        fallbackIntro: (min: number) =>
            `Оценок пока мало (сериалу нужно хотя бы ${min}), поэтому пока здесь то, что чаще всего смотрят и досматривают.`,
        fallbackHeading: "Самое смотримое на MyBLHub",
        watchers: (n: number) => `${n} ${plural(n, ["зритель", "зрителя", "зрителей"])}`,
        empty: "Оценок пока нет — поставьте первую на странице сериала.",
        // Тупика в конце списка быть не должно: он короткий по
        // построению, и человеку есть куда идти дальше (правка
        // владельца 2026-09-10).
        searchMore: "Искать сериалы по фильтрам",
    },
