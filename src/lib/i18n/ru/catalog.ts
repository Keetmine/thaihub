...
export const catalog: Dict["catalog"] = {
    ...
    drama: {
         ...
         *  других сайтах», без точного числа наших оценок: пока их
         *  единицы, «(2 оценки)» скорее отпугивает. Когда источник один,
         *  честно называем его. */
        scoreTooltip: (site: string | null, mdl: string | null) => {
            if (site && mdl) {
                return `Рейтинг сериала: оценки зрителей MyBLHub — ${site}, MyDramaList — ${mdl}`;
            }
            if (site) return `Рейтинг сериала по оценкам зрителей MyBLHub — ${site}`;
            return `Рейтинг сериала по оценкам MyDramaList — ${mdl}`;
        },
        events: "События",
