...
export const account: Dict["account"] = {
  ...
  telegramPrompt: {
      ...
      intro: "Вы зарегистрировались по почте — уведомления пока приходят только на сайт, и о них легко не узнать. Привяжите Telegram, и мы напишем туда:",
      pointEpisodes: "вышла новая серия сериала, за которым вы следите",
      pointFriends: "кто-то добавил вас в друзья или позвал в поездку",
      pointPresale: "открылась продажа билетов на событие, куда вы идёте",
      privacy: "Что именно присылать — вы выбираете в настройках, там же связь отключается.",
      later: "Потом",
      linkFailed: "Не получилось привязать — попробуйте из настроек, там есть подробности",
  },
  notifications: {
